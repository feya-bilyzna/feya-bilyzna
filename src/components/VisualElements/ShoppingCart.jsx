...
`
    ...
    console.log(cookies.cartProducts ? Object.values(cookies.cartProducts).map(variantData => variantData) : [])
    // console.log(Object.values(data.productsByIds).map(pr => Object.values(pr).map(p=>p)))
    console.log(Object.values(data.productsByIds).map(pr => pr))
    ...
    return <>
        ...
        {cookies.cartProducts ? Object.keys(cookies.cartProducts).map(cookieVariantData =>
            Object.values(data.productsByIds).map(pr => pr.remains.map(variant => variant.variantId == cookieVariantData ?
                <Row style={{display: "flex", flexWrap: "wrap", marginBottom: 0}}>
